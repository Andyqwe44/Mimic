...
```
tictactoe/
├── .cursor/rules/            # Cursor 项目规则 (.mdc) — 铁律/后端/前端/构建
...
4. git push origin + tag；**GitHub mirror best-effort**（`git push github` 失败不阻断 Gitee 发版）
5. `Publish.ps1`（`Invoke-RestMethod` 建 Gitee Release + 手搓 multipart 传 installer）→ 验 raw URL 302→200
