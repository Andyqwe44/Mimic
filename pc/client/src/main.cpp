/**
 * mimic_client — Pure C++ WebView2 host for Mimic Client.
 *
 * One process: Win32 window + WebView2 + capture + peer signaling client.
 * Prod: build\bin\mimic_client.exe → https://gam.local/index.html (frontend on disk).
 */
#include <windows.h>
#include <objbase.h>
...
static constexpr PCWSTR TITLE        = L"Mimic Client";
static constexpr PCWSTR WINDOW_CLASS = L"MimicClient";
...
static constexpr PCWSTR SINGLE_INSTANCE_MUTEX = L"Global\\MimicClient_8A3F2D";
...
static void heal_local_manifest() {
    ...
    if (GetFileAttributesA((installDir + "\\bin\\mimic_client.exe").c_str()) == INVALID_FILE_ATTRIBUTES) {
...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE, _In_ LPSTR lpCmdLine, _In_ int nCmdShow)
{
    ...
    LOG("main", "MimicClient starting (auto_stream=%d)", (int)auto_stream);
    ...
    // Append elevation to window title — "Mimic Client (管理员)"
    {
        BOOL isAdmin = FALSE;
...
static void show_main_window()
{
    ...
    {
        const wchar_t* help =
            L"Mimic Client 未能正常加载。\r\n\r\n"
            L"这可能是因为 WebView2 环境创建失败,或前端资源损坏。\r\n\r\n"
            L"请尝试:\r\n"
            L"1. 重新安装 Mimic Client\r\n"
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_SELFTEST_EVENT:
        ...
        break;
    case WM_PEER_UI_EVENT:
        // peer WS/LAN reader → STA → WebView (devices / invite / frames).
        peer_ui_drain_to_webview();
        break;
    case WM_DESTROY:
...
HRESULT InitWebView2(HWND hwnd)
{
    ...
    // the folder to next-to-the-exe (e.g. C:\Program Files\MimicClient\bin\),
        ...
        new EnvCreatedHandler([hwnd](HRESULT result, ICoreWebView2Environment* env) -> HRESULT {
            if (FAILED(result)) {
                ...
                MessageBoxW(hwnd,
                    ...
                    L"Mimic Client", MB_ICONERROR | MB_OK);
                return result;
            }
            ...
            return env->CreateCoreWebView2Controller(hwnd,
                new ControllerCreatedHandler([hwnd](HRESULT result, ICoreWebView2Controller* ctrl) -> HRESULT {
                    ...
                    // Map virtual host to frontend folder on disk.
