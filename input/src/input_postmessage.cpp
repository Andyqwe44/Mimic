    ...
    if (a.type == "click" || a.type == "dblclick") {
        ...
        auto doClick = [&]() -> bool {
            if (!PostMessageW(hWnd, downMsg, wDown, lp)) {
                LOG("input","PostMessage down failed hwnd=0x%llx",(unsigned long long)(uintptr_t)hWnd);
                return false;
            }
            Sleep(10);
            if (!PostMessageW(hWnd, upMsg, 0, lp)) {
                LOG("input","PostMessage up failed hwnd=0x%llx",(unsigned long long)(uintptr_t)hWnd);
                return false;
            }
            return true;
        };
        if (!doClick()) return "{\"ok\":false,\"error\":\"PostMessage failed (target window may be destroyed)\"}";
        ...
        { int cx,cy; norm_to_client(hWnd,a.dragPath[0].first,a.dragPath[0].second,cx,cy);
          ...
          PostMessageW(hWnd,dM,wD,MAKELPARAM(cx,cy)); }
        for (size_t i=1;i<a.dragPath.size();i++) { int cx,cy; norm_to_client(hWnd,a.dragPath[i].first,a.dragPath[i].second,cx,cy);
          PostMessageW(hWnd,WM_MOUSEMOVE,wD,MAKELPARAM(cx,cy)); Sleep(5); }
        { int cx,cy; norm_to_client(hWnd,a.dragPath.back().first,a.dragPath.back().second,cx,cy);
          PostMessageW(hWnd,uM,0,MAKELPARAM(cx,cy)); }
    }
    else if (a.type == "wheel") {
        int delta = -a.delta;
        PostMessageW(hWnd, WM_MOUSEWHEEL, MAKEWPARAM(0, (short)delta), MAKELPARAM(clientX, clientY));
    }
        ...
        auto dk = [&](bool up) {
            ...
            if (up) lp |= (1 << 31) | (1 << 30);
            PostMessageW(hWnd, msg, (WPARAM)a.vk, lp);
        };
        if (a.type == "keypress") { dk(false); Sleep(5); dk(true); }
        else { dk(a.type == "keyup"); }
    }
        ...
        auto dk = [&](WORD kv, bool up) {
            ...
            if (up) lp |= (1 << 31) | (1 << 30);
            PostMessageW(hWnd, msg, (WPARAM)kv, lp);
        };
        ...
        for (int i = 0; i < wl; i++) {
            PostMessageW(hWnd, WM_CHAR, wb[i], MAKELPARAM(1, 1));
            Sleep(5);
        }
    }
